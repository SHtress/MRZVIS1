// mrzvis1.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
vector<bool> Sum(vector<bool> A, vector<bool> B) {
	...
	//cout << printVector(A) << " +"<<printVector(B)<<" = ";
	...
	row++;
	column++;
	//if (currentTick % 2 == 0) cout << "Tick=" <<currentTick / 2 << endl;
	return result;
}
...
vector<bool> Multiply(vector<bool> A, vector<bool> B) {
	...
	{	
		...
		row++;
		column++;
		//cout<<printVector(B)<<" >>"<<printVector(sdwig)<<endl;
		currentTick++;
...
int main()
{
	...
	//Multiply(listA[length - 1], listB[length - 1]);
	cout << "---------" << endl;
